...
- **Regional Managed Instance Group (`google_compute_region_instance_group_manager.frontend_mig`)**: Manages the lifecycle of our frontend instances across multiple zones in the region. It ensures that the specified number of instances are running and automatically replaces failed ones, ensuring high availability. It is configured with an auto-healing policy to automatically recreate instances that fail the primary health check.
- **Autohealing Health Check (`google_compute_region_health_check.autohealing`)**: A regional HTTP health check used specifically by the managed instance group's auto-healing policy to continuously monitor instances on port 80.
